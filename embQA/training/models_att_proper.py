...
class VqaLstmCnnAttentionModel(nn.Module):
    ...
    def forward(self, images, questions, ques_string):
        ...
        ques_feats_tr = ques_feats_repl
        ques_feats_tr = ques_feats_tr.unsqueeze(2).unsqueeze(3)
        ques_feats_tr = ques_feats_tr.repeat(1,1,10,10)
        ques_img_feats = torch.cat([ques_feats_tr, img_feats_tr], 1)
        # ques_img_feats = ques_feats_tr * img_feats_tr

        # att_feats = self.att(ques_img_feats)
        att_feats = self.conv_att(ques_img_feats)
        att_probs = F.softmax(att_feats.view(N*T,1,10*10), dim=2).view(N*T,1,10,10)
        # att_probs = nn.Softmax2d()(att_feats)
        # att_probs2 = att_probs.view(N, T, 1).repeat(1, 1, 64)

        att_img_feats = torch.mul(att_probs.repeat(1,64,1,1), img_feats)
        att_img_feats = torch.sum(att_img_feats, dim=(2,3))

        mul_feats = torch.mul(ques_feats, torch.sum(att_img_feats.view(N,T,64),dim=1))
        scores = self.classifier(mul_feats)

        att_images = att_probs.view(N,T,1,10,10).squeeze(2)
        for example_idx,example in enumerate(att_images):
            for position_idx,position in enumerate(example):

                position = position.view(10,10)
                cam = position.cpu().data.numpy()
                ...
                cam = heatmap + np.float32(images[example_idx][position_idx].permute(1,2,0))
                cam = cam / np.max(cam)
                cam = np.uint8(255 * cam)
                cv2.imwrite("att_images/"+str(example_idx)+"_"+str(position_idx)+"_cam.png",cam)
                cv2.imwrite("att_images/"+str(example_idx)+"_"+str(position_idx)+"_img.png",np.uint8(np.float32(images[example_idx][position_idx].permute(1,2,0))*255))
        with open("att_images/ques.txt",'w') as f:
            f.write(str(ques_string)) 
...
class NavCnnModel(nn.Module):
    ...
    def forward(self, img_feats, questions=None):
        ...
        #print("Img Features:", img_feats.size())

        if self.question_input == True:
            ques_feats = self.q_rnn(questions)
            ques_feats = self.ques_tr(ques_feats)
            # print(ques_feats.size())
