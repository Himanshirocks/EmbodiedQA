...
def build_mlp(input_dim,hidden_dims,output_dim,use_batchnorm=False,dropout=0,add_sigmoid=1):
    layers = []
    D = input_dim
    if dropout > 0:
...
class BottleSlaveRnn(nn.Module):
    def __init__(self,
                 image_feat_dim=256,
                 question_embed_dim=64,
                 num_actions=3,
                 rnn_hidden_dim=128,
                 master_hidden_dim=1024,
                 rnn_num_layers=2,
                 rnn_dropout=0,
                 return_states=False):
        super(BottleSlaveRnn, self).__init__()
        ...
        self.rnn_hidden_dim = rnn_hidden_dim
        self.rnn_num_layers = rnn_num_layers
        self.master_hidden_dim = master_hidden_dim
        self.return_states = return_states

        # Add Convolutional stuff here
        self.ques_tr = nn.Linear(question_embed_dim, question_embed_dim)

        self.img_downsample = nn.Sequential(nn.Conv2d(32 + question_embed_dim,128,4,2,1),
                                            nn.BatchNorm2d(128),
                                            nn.LeakyReLU(0.01),
                                            nn.Conv2d(128, image_feat_dim, 5, 1, 0))


        rnn_input_dim = image_feat_dim + master_hidden_dim
        ...
        print('Building GRU with hidden dim: %d' % (rnn_hidden_dim))
    ...
    def forward(self, img_feats, ques_feats, action_lengths, master_hidden_feats):
        
        N, T, c, h,w = img_feats.size()
        # import pdb;pdb.set_trace()
        ques_feats = self.ques_tr(ques_feats)
        N, D = ques_feats.size()
        ques_feats = ques_feats.view(N, 1, D, 1, 1)    
        ques_feats = ques_feats.repeat(1, T, 1, h , w)

        img_feats = img_feats.view(N * T, c, h, w)
        ques_feats = ques_feats.view(N * T, D, h, w)
        input_feats = torch.cat([img_feats, ques_feats], dim =1)

        ques_img_feats = self.img_downsample(input_feats).view(N, T, self.image_feat_dim)

        # # Convert to One Hot
        # actions_OH = Variable(oneHot_vol(actions_in, 3).cuda())
        # input_feats = torch.cat([ques_img_feats, actions_OH], 2)
        # # ^ This is working!

        input_feats = torch.cat([ques_img_feats, master_hidden_feats], dim=2)
...
class BottleMasterGRU(nn.Module):
    def __init__(self,
                 question_vocab,
                 num_output=2,
                 question_wordvec_dim=64,
                 ...
                 rnn_hidden_dim=1024,
                 rnn_num_layers=1,
                 ques_image_feat_dim=256,
                 rnn_dropout=0,
                 num_actions=3):
        super(BottleMasterGRU, self).__init__()
        ...
        self.img_downsample = nn.Sequential(nn.Conv2d(32 + question_hidden_dim, 128, 4, 2, 1),
                                            nn.BatchNorm2d(128),
                                            nn.LeakyReLU(0.01),
                                            nn.Conv2d(128, ques_image_feat_dim, 5, 1, 0))
        self.ques_image_feat_dim = ques_image_feat_dim

        rnn_input_dim = ques_image_feat_dim + num_actions + 4
        self.rnn_hidden_dim = rnn_hidden_dim
        self.rnn_num_layers = rnn_num_layers
    ...
    def forward(self,questions,img_feats,  prev_actions, action_lengths, pos_queue):
        ...
        ques_feats = self.ques_tr(ques_feats)
        N, D = ques_feats.size()
        question_feats = ques_feats.view(N, 1, D, 1, 1)    
        question_feats = question_feats.repeat(1, T, 1, h , w)

        img_feats = img_feats.view(N * T, c, h, w)

        question_feats = question_feats.view(N * T, D, h, w)
        input_feats = torch.cat([img_feats, question_feats], dim =1)
        # import pdb;pdb.set_trace()
        ques_img_feats = self.img_downsample(input_feats).view(N, T, self.ques_image_feat_dim)
        
        # Satty: Get OH volume (Batch_size, Time Steps, num_actions)
        prev_action_OH = Variable(oneHot_vol(prev_actions, 3).cuda())
        input_feats = torch.cat([ques_img_feats, prev_action_OH, pos_queue], dim = 2)
        packed_input_feats = pack_padded_sequence(input_feats, action_lengths, batch_first=True)
        packed_output, hidden = self.rnn(packed_input_feats)
        output, _ = pad_packed_sequence(packed_output, batch_first=True)
        ...
        return scores, output, ques_feats
