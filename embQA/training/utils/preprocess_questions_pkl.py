...
def checkObjs(house_dir, houseID):
    objFile = os.path.join(house_dir, 'house', houseID, 'house.obj')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-suncg_data_dir', required = True, type=str)
    ...
    for houseID in list(data['questions'].keys()):
        if not checkObjs(args.suncg_data_dir, houseID):
            ...
            if houseID in data['splits']['test']:
                data['splits']['test'].remove(houseID)
            assert houseID not in data['questions'].keys()
